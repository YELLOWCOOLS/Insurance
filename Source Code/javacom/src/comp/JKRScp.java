...
public class JKRScp extends Cpimpl implements Cp{
	...
	public JKRScp() {
		...
		//一个开始对应一个结束
		Zhongjistart = new String[]{"9.7    轻症疾病|9.7    重大疾病|9.10   轻症疾病|7.5    重大疾病|10.6   重大疾病|7.3     重大疾病|7.5     重大疾病|9.4    重大疾病|10.7   重大疾病|777.7...5555 重大疾病|8.5       重大疾病|9.49.49.4 重大疾病 "};
		Zhongjiend = new String[]{"重大疾病定        1.六项基本日常生|9.8    医院|9.12   全残|7.6    毒品|10.9   医院|7.4     医院|7.6     医院|9.5    调整系数|10.8   意外伤害|7.67.67.6 毒品 |8.6       周岁|9.59.59.5 增值系数 "};
		...
		if (line == null) {
			...
			case 6:
				do{
					...
					ma1 = p0000.matcher(line);
					if (ma1.find()&&!Pattern.compile("^[ ]{0,15}[0-9]+\\.[0-9]+\\.[0-9]+\\.").matcher(line).find()) {
						inf = 5;
						illlist.add(name.toString(), exp.toString());
						break;
					}
					...
					if (ma1.find()&&!Pattern.compile("^[ ]{0,15}[0-9]+\\.[0-9]+\\.[0-9]+\\.").matcher(line).find()) {
						...
						exp = new StringBuffer(" ");
						tmp = line.trim();
						String tmps[] = tmp.replaceAll("p1G", "").split("[ ]{2,}");

...
public class TFQQRScp extends Cpimpl implements Cp{
	...
	public TFQQRScp() {
		...
		//一个开始对应一个结束
		Zhongjistart = new String[]{"6.7  重大疾病|6.5  重大疾病|5.10 重大疾病|10.7 重大疾病|8.1  重大疾病|6.1  重大疾病|10.10  重大疾病|8.2  重大疾病|6.1    重大疾病|6.1  特定重大疾病|重大疾病指下列所定义的三十六项疾病或手术|7.1  重大疾病|5.1  重大疾病|5.1   重大疾病|5.1  特定重大疾病","^[ ]{0,15}[0-9]+\\.[0-9]+[ ]{0,}保险责任"};
		Zhongjiend = new String[]{"6.9  指定或认可的医院|10.9 重大疾病分组|6.9    六项基本日常生活活动|5.14   六项基本日常生活活动|10.10 六项基本日常生活活动|8.5  六项基本日常生活活动|6.4  六项基本日常生活活动|10.14  六项基本日常生活活动|8.6  六项基本日常生活活动|6.4    六项基本日常生活活动|6.4   六项基本日常生活活动|6.5  六项基本日常生活活动|7.4  六项基本日常生活活动 |5.4  六项基本日常生活活动 |5.2   肢体机能完全丧失 |5.5  六项基本日常生活活动","^[ ]{0,15}[0-9]+\\.[0-9]+[ ]{0,}责任免除"};
...
//				break;
	...
	@Override
	public int dutyprocess(BufferedReader br) throws IOException {
		
		Pattern dutystart = Pattern.compile(Zhongjistart[1]);
		Pattern dutyend = Pattern.compile(Zhongjiend[1]);
		...
		while((tmp=br.readLine())!=null){
					...
					if(dutystart.matcher(tmp).find()&&!flag)
					{
						flag=true;
						name.append("");
						inf=0;
					...
					if(flag){
						if(dutyend.matcher(tmp).find())
						{
							inf=0;
							flag=false;
							illlist.add(name.toString(),exp.toString().trim());
						}else {
							exp.append(tmp.trim());
						}
					}
...
//				continue;
			...
			if (Pattern.compile("\\s+本\\s+页\\s+是\\s+空\\s+白|重大疾病保险合同”。").matcher(line).find()) {
				nameflag = flag = false;
				inf += 1000;
				break;
			}
...
//				break;
			...
			case 7:
				do {
					if(p000.matcher(line).find())
					{
						do{
							exp.append(line);
						}while(p0000.matcher(line).find());
					}
					...
					if (ma1.find()&&(!p1G.matcher(line).find())) {
						...
						name.append(tmp.replaceAll("^[ ]{0,15}+[0-9]+\\.[0-9]+|\\s+",""));
					} else 
			...
			case 10:
				do {
					...
					if (ma1.find()) {
						inf = 5;
						illlist.add(name.toString().trim(), exp.toString().trim());
						break;
					}

...
public class ZYRScp extends Cpimpl implements Cp {
		...
		Mulustart = "条款目录";
		Muluend ="中意";
		Zhongjistart = new String[]{"第[一二三四五六七八九十]+条[ ]*重大疾病保障范围及定义|重大疾病的定义及范围|[0-9]+[ ]*重大疾病保障范围及定义"};
		Zhongjiend = new String[]{"释义"};
		...
		public int Zhongjiprocess(BufferedReader br) throws IOException {
			...
			if (line == null) {
				...
				while ((line = br.readLine()) != null) {
					...
					if(Pattern.compile("第[一二三四五六七八九十]组").matcher(line).find()) {
						continue;
					}
				...
				case 7:
					do {
						...
						if (ma1.find()) {
							...
							exp = new StringBuffer(" ");
							tmp = line.trim();
							String tmp2=line.trim();
							String tmp3=line.trim();
							String tmps[] = tmp.split("[ ]{3,}");
							if(tmps.length==1) {
								tmps=tmp2.split("[ ]{2,}");
								if(tmps.length==1) {
									tmps=tmp2.split("[ ]{1,}");
			...
			if (line == null) {
				...
				case 7:
					do {
						...
						if (Pattern.compile("^[0-9]+[ ]{0,}重大疾病").matcher(line).find()) {
							endflag=true;
							illlist.add(name.toString().replaceAll("\\s+", ""), exp.toString().replaceAll("\\s+", ""));
							name = new StringBuffer(" ");
							exp = new StringBuffer(" ");

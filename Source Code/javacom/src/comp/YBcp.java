...
public class YBcp extends Cpimpl implements Cp{
	...
	public YBcp() {
		...
		//一个开始对应一个结束
		Zhongjistart = new String[]{"重大疾病：|三、第一类重大疾病：|一、重大疾病：|四、第一类重大疾病|四、重大疾病：","第[一二三四五六七八九十]条[ ]{0,}保险责任"};
		Zhongjiend = new String[]{"利息：|未满期净保险费：|九、医院：|、六项基本日常生活活动|五、毒品：|友邦附加护身符重大疾病保险","第[一二三四五六七八九十]条[ ]{0,}责任免除"};
...
//					if (ma1.find()) {
//						inf = 5;
//						illlist.add(name.toString().trim().replaceAll("\\s+",""), exp.toString().trim().replaceAll("\\s+",""));
//						break;
//					}

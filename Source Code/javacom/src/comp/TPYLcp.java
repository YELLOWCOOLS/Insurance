...
public class TPYLcp extends Cpimpl implements Cp{
	...
	public TPYLcp() {
		...
		//一个开始对应一个结束
		Zhongjistart = new String[]{"本合同所指重大疾|本附加合同所指少","第[一二三四五六七八九十]条[ ]{0,}保险责任"};
		Zhongjiend = new String[]{"六项基本日常生活          ： |六项基本日常生活          ：   （1）穿衣：自己能够穿衣及脱衣；","第[一二三四五六七八九十]条[ ]{0,}责任免除"};
			...
			case 8:
				do {
					...
					if (ma1.find()) {
						...
						nameflag = false;
					} else if (p1I1.matcher(line).find()) {
							if (!flag) {
...
//				continue;
			...
			if (Pattern.compile("解除保险合同会造成一定的损失，请留意").matcher(line).find()) {
				nameflag = flag = false;
				inf += 1000;
				break;
			}

...
public class ZHLHRScp extends Cpimpl implements Cp {
	...
	public ZHLHRScp() {
		...
		// 一个开始对应一个结束
		Zhongjistart = new String[] { "^[ ]*[0-9]+.[0-9]+[ ]*重大疾病|^[ ]*[0-9]+.[0-9]+[ ]*重大疾病定义" };
		Zhongjiend = new String[] { "[0-9].[0-9]  [ ]*释义|[0-9]. [ ]*释义|10.3   特定严重疾病" };
			...
			case 4:
				do {
					...
					if (ma1.find()) {
						...
						exp = new StringBuffer("");
						String results[] = line.split("、");
						ma1=Pattern.compile(":").matcher(line);
						boolean we=ma1.find();
						String resultss[] = null;
						if(we) {
							 resultss = results[1].split(":");
			...
			case 6:
				do{
					...
					if (ma1.find()) {
						...
						if(Pattern.compile("1").matcher(name).find()) {
							name.append(tmps[2].trim());
						}
						if (tmps.length >= 3)
						...
						ma1 = p2.matcher(line.substring(0, 14));
						if (ma1.find() && nameflag) {

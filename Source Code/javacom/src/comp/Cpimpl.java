...
public class Cpimpl implements Cp {
	...
	public String Mulustart = "条款目录";
	public String Muluend = "东吴附加定期重大疾病保险条款";
	public String[] Zhongjistart = { "重大疾病：" };
	public String[] Zhongjiend = { "null" };

	public Cpimpl() {

	}

	public Cpimpl(Cpimpl impl) {
		this.Mulustart = impl.Mulustart;
		this.Muluend = impl.Muluend;
		this.Zhongjistart = impl.Zhongjistart;
		this.Zhongjiend = impl.Zhongjiend;
		this.illlist = new linkList();
	}
...
public void Letin(String id) throws Exception {
		
		
		illNode node=this.illlist.head;
		...
		while(node!=null) {
			if(node.type==18) {
//				num++;
//				sql="INSERT zy_tag(id,type,tag2) VALUES(?,?,?)";
//				Object[] param1 = {id,2,node.Name};
//				JdbcTemplate.update(sql, param1);
//				tag.append("|"+node.Name);
			}else {	
//			sql="INSERT zy_detailexp(id,name,exp) VALUES(?,?,?)";
//			Object[] param2 = {id,node.Name,node.Key};
//			JdbcTemplate.update(sql, param2);		
			 sql="INSERT zy_illlist(id,illname,illexp) VALUES(?,?,?)";
			 Object[] params = {id,node.Name,node.Key};	
//				JdbcTemplate.update(sql, params);
			}
			node=node.next;
		}
//		sql = "UPDATE zy_tag_count set numoftag2=?,tag2=?  where id=?";
//		Object[] param3= {num,tag.toString(),id};

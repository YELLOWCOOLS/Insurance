...
public class ABYLcp extends Cpimpl implements Cp {
	...
	public ABYLcp() {
		...
		// 一个开始对应一个结束
		Zhongjistart = new String[] { "6.5    重大疾病" };
		Zhongjiend = new String[] { "6.6    毒品" };
	...
	public int Zhongjiprocess(BufferedReader br) throws IOException {
		...
		Pattern p1C = Pattern.compile("^[ ]{0,15}[一二三四五八九十]+[ ]+"); // 一 恶行肿瘤 释义
		Pattern p1D = Pattern.compile("^[ ]{0,15}（[一二三四五八九十]+"); // （一） 病名
		Pattern p1E = Pattern.compile("^[ ]{0,15}[一二三四五八九十]+、"); // 一、疾病名 释义
			...
			case 3:
				do {
					...
					if (ma1.find()) {
						...
						if (line.length() > 10 && !p2.matcher(line.substring(0, 10)).find()) {
							nameflag = false;
						} else {
							nameflag = true;
						}

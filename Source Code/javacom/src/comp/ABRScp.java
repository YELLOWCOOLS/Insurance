...
public class ABRScp extends Cpimpl implements Cp{
	...
	public ABRScp() {
		...
		//一个开始对应一个结束
		Zhongjistart = new String[]{"7.3     重大疾病|6.4    重大疾病|6.3   重大疾病|6.3      重大疾病|6.10   轻症疾病|6.3     重大疾病|6.3   少  儿 重 大|6.7     重大疾病|6.5    重大疾病|6.6    重大疾病|6.4   重大疾病"};
		Zhongjiend = new String[]{"7.4|6.5|6.4   意外伤害|6.4      意外伤害| 6.12   毒品 |6.4     意外伤害|6.4   意外伤害 |6.8     意外伤害|6.6    意外伤害|6.7    意外伤害|6.5   医院"};
				...
				do{
					...
					if (ma1.find()) {
						...
						if (nameflag) {
							...
							ma1 = p2.matcher(tmp.substring(0, 8));
							if (ma1.find()) {
								line = line.trim();
								tmp = line.split("[ ]{2,}|  ")[0];
								name.append(tmp);
...
//				break;
	...
	public int allprocess(BufferedReader br)throws Exception{
		...
		Pattern p0 = Pattern.compile(Muluend);
		Pattern p000 = Pattern.compile(Zhongjistart[0]);
		Pattern p0000 = Pattern.compile("^[ ]{0,}[0-9]+\\.[0-9]+永久不可逆");
		Pattern p2 = Pattern.compile("[\u4e00-\u9fa5]");
		Pattern p3 = Pattern.compile("第[ ]*[0-9]+[ ]*页");

...
public class NYRS extends Cpimpl implements Cp {
	...
	public NYRS() {
		...
		Mulustart = "条款目录 ";
		Muluend ="嘉禾|农银";
		Zhongjistart = new String[]{"^[0-9].[0-9]  [ ]*重大疾病|^[[0-9]+.]+[ ]*[0-9]+[ ]*重大疾病"};
		Zhongjiend = new String[]{"[0-9].[0-9] [ ]*意外伤害 |[0-9].[0-9] [ ]*毒品"};
		...
		public int Zhongjiprocess(BufferedReader br) throws IOException {
			...
			Pattern p1B = Pattern.compile("^[ ]{0,25}[0-9]+、"); // 1、疾病名 释义
			Pattern p1C = Pattern.compile("^[ ]{0,15}[一二三四五六七八九十]+[ ]+"); // 一 恶行肿瘤 释义
			Pattern p1D = Pattern.compile("^[ ]{0,15}（[一二三四五六七八九十]+"); // （一） 病名
			Pattern p1E = Pattern.compile("^[ ]{0,40}[一二三四五六七八九十]+、"); // 一、疾病名 释义

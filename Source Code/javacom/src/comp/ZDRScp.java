...
public class ZDRScp extends Cpimpl implements Cp {
		...
		Zhongjistart = new String[]{"病定义使用规范"};
		Zhongjiend = new String[]{"第[一二三四五六七八九十]部分[ ]*如何支付保险费|[一二三四五六七八九十]、重大疾病组分类|^[ ]*释义|[[0-9]+.]+[ ]*周岁|第[一二三四五六七八九十]+条[ ]+受益人指定和变更"};
		...
		public int Zhongjiprocess(BufferedReader br) throws IOException {
			...
			Pattern p00000=Pattern.compile("中德|安联");
			...
			Pattern p1J = Pattern.compile("^[ ]{0,15}[A]*[0-9]+[ ]*．"); // 1．疾病名 释义                    1                      9
					...
					do{
						...
						if (ma1.find()) {
							...
							if (we&&ma1.start()<30) {
								...
								resultss = results[1].trim().split("[ ]{1,}");
							}
							name.append(resultss[0].trim());

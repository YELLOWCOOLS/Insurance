...
public class JLRScp extends Cpimpl implements Cp{
	...
	public JLRScp() {
		...
		Mulustart = "条款目录|条 款 目 录|条 款     目 录|条   款   目   录|条 款     目 录";
		Muluend ="重大疾病保险";
		//此处设置为数组  表示重疾所在的段落可能不在 一个地方
		//一个开始对应一个结束
		Zhongjistart = new String[]{"[ ]{0,30}与中国医师协会制定的《重大疾病保险疾病定义使用规范》所规范的重大疾|9.4    危重疾病|9.4    重大疾病|9.4    重大疾病|8.4   重大疾病| 9.6  重大疾病|9.6   重大疾病|8.4    重大疾病|9.4  重大疾病|9.4  重大疾病|8.4  重大疾病|6.6   重大疾病|9.4  重大疾病"};
		Zhongjiend = new String[]{"9.5   意外伤害|9.5    意外伤害事故|9.6    意外伤害事故|9.6    意外伤害事故|8.5   意外伤害事故|9.6   意外伤害事故|9.5   意外伤害事故|9.7  毒品|9.7   毒品|8.5    意外伤害事故| 9.5  意外伤害|9.6  意外伤害事故|8.5  意外伤害事故|6.7   意外伤害|9.5  意外伤害事故"};
	...
	public void defaultpro(BufferedReader br) throws IOException {
		...
		while ((line = br.readLine()) != null) {
			...
			if(Pattern.compile("君龙附加龙安康提前给付重大疾病保险条款|君龙附加真爱重大疾病保险条款|君龙康祥重大疾病保险条款|君龙附加豁免保费重大疾病保险条款|君龙附加享乐一生提前给付重大疾病保险条款|君龙附加额外给付重大疾病保险条款|9.5+[ ]{0,30}+特定疾病+[ ]{0,30}+指符合下列定义的疾病:|君龙附加晶彩安康提前给付重大疾病保险条款").matcher(line).find()) {
				nameflag=false;
				continue;
			}
	...
	public int Zhongjiprocess(BufferedReader br) throws IOException {
		...
		Pattern p1F = Pattern.compile("^[ ]{0,15}[0-9]+\\."); // 1.疾病名 释义
		...
		Pattern p1I1 = Pattern.compile("^[ ]{0,15}+[0-9]+[)]");
		...
		if (line == null) {
			...
			while ((line = br.readLine()) != null) {
				...
				if ((!hasfindpinStart)&(p1I.matcher(line).find()||p1I.matcher(line).find())) {
					plan = 8;
					break;
				}
...
//				continue;
		...
		if (line == null) {
			...
			case 10:
				do {
					if(Pattern.compile("    （4）争议处理。").matcher(line).find()) {
						int i=1;
					}
					if (p3.matcher(line).find()) {
						continue;

...
public class TPRScp extends Cpimpl implements Cp{
	...
	public TPRScp() {
		...
		//一个开始对应一个结束
		Zhongjistart = new String[]{"重大疾病的种类及定义|本合同所指重大疾"};
		Zhongjiend = new String[]{"本页内容结束|六项基本日常生活          ：    "};
...
//				continue;
			...
			if (Pattern.compile("第2页，").matcher(line).find()) {
				nameflag = flag = false;
				inf += 1000;
				break;
			}
			...
			case 10:
				do {
					...
					if (ma1.find()) {
						...
						name = new StringBuffer("");
						exp = new StringBuffer("");
						if(Pattern.compile("第七条“保险责任”第四款处理。|第十六条所定义的终末期疾病，").matcher(line).find()) {
							exp.append(line.replaceAll("\\s+",""));
						}else{
						String results[] = line.split("[ ]{1,}");
						//String rest[] = results[1].trim().split("[ ]{2,}");
						name.append(results[1].replaceAll("\\s+",""));

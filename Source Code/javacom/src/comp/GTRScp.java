...
public class GTRScp extends Cpimpl implements Cp{
	...
	public GTRScp() {
		...
		//一个开始对应一个结束
		Zhongjistart = new String[]{"3.2 重大疾病"};
		Zhongjiend = new String[]{"4.责任免除 "};
				...
				do {
					...
					if (ma1.find()) {
						if (!flag) {
							...
							inf = 4;
							illlist.add(name.toString().replaceAll("[0-9]+\\.[0-9]+|\\s+", ""), exp.toString().replaceAll("\\s+", ""));
						}
						name = new StringBuffer(" ");
						exp = new StringBuffer(" ");
						tmp = line.trim();
						
						name.append(line.trim().replaceAll("[0-9]+\\.[0-9]+|\\s+", ""));

...
public class GYASRScp extends Cpimpl implements Cp{
	...
	public GYASRScp() {
		...
		Mulustart = "在您阅读本条款之前|条款目录";
		Muluend ="本附加合同。|工银安盛人寿";
		//此处设置为数组  表示重疾所在的段落可能不在 一个地方
		//一个开始对应一个结束
		Zhongjistart = new String[]{"4 重大疾病|轻症：：：|4 重大疾病：|5 重大疾病：|2 重大疾病：|4 重大疾病：|重大疾病（4）          |重大疾病（3） |重大疾病（3）           ：|重大疾病（6）          |A类重大疾病            ：|A类重大疾病（5）         ：|重大疾病              1.|重大疾病              1.|重大疾病（5）        ：|重大疾病（3）           1|重大疾病（7）          :|重大疾病               1. "};
		Zhongjiend = new String[]{"7 意外伤害事故：|现金价值：|5 意外伤害事故：|6 意外伤害事故：|3 专科医生：|5 现金价值：|毒品（9）             ：|专科医生（4）|专科医生（4）           ：|未满期保险费         ：|毒品（7）             ：|毒品                ：|专科医生              ：|专科医生（4）           ：|毒品                ：|专科医生（4）           ：|约定期限（8）        ：|专科医生（4）           ："};
	...
	public int Zhongjiprocess(BufferedReader br) throws IOException {
		...
		Pattern p1B = Pattern.compile("^[ ]{0,20}[0-9]+、"); // 1、疾病名 释义
			...
			case 5:
				do{
					...
					if (ma1.find()) {
						...
						name.append(results[results.length-1]);

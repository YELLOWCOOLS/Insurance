...
public class BDFZRScp extends Cpimpl implements Cp {
	...
	public BDFZRScp() {
		Mulustart = "目录";
		Muluend = "海尔纽约人寿|第[ ]*1[ ]*页";
		Zhongjistart = new String[] { "重大疾病是指下列任何一种疾病" };
		Zhongjiend = new String[] { "重大疾病定义的释义|专科医生应当同时满足以下四项资格条件" };
	}
		...
		if (line == null) {
			...
			case 10:
				do {
					...
					} else if (Pattern.compile("^[ ]{0,}+第八部分+[ ]{0,}+释义").matcher(line).find()) {
						illlist.add(name.toString().replaceAll("\\s+", ""), exp.toString().replaceAll("\\s+", ""));
						name = new StringBuffer("");
						exp = new StringBuffer("");

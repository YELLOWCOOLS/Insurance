            ...
            <table id="datatable" class="table table-striped table-hover">
              ...
              <tbody>
                <?php if(is_array($ins)): $i = 0; $__LIST__ = $ins;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$in): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td>
                      <a class="btn btn-xs btn-default" href="javascript:;" data-update="<?php echo ($in["id"]); ?>">详情 </a>&nbsp;
                      <a class="btn btn-danger btn-xs" href="javascript:;" data-remove="<?php echo ($in["num"]); ?>"><i class="fa fa-trash"></i></a></td>
  ...
  $(function() {
    ...
    $('#datatable').on('click', '[data-remove]', function() {
      ...
      swal({
        ...
        function() {
          $.post('/admin/candidate/deletein', { num: dateId }, function(res) {
             swal({
                title: "删除成功",
                type: "success",
              }, function() {
    ...
    $('#datatable').on('click', '[data-update]', function() {
      var dateId = $(this).data('update');
      　window.location.href="/admin/candidate/insuranview/id/"+dateId;
    });
